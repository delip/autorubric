...
- `JudgeVote.error` / `MultiChoiceJudgeVote.error` / `EnsembleCriterionReport.error` carry category-prefixed messages; `eval.py` serialization round-trips `error` on ensemble reports, binary judge votes, and multi-choice judge votes
- `CriterionReport.reasoning` / `JudgeVote.reasoning` / `MultiChoiceJudgeVote.reasoning` carry the extended-thinking deliberation trace (T6-B; see *Grading Flow*); serialization round-trips it symmetrically with `error` — single-report path via pydantic `model_dump`/`model_validate`, ensemble path via explicit per-vote dicts (`.get("reasoning")` → `None` on legacy checkpoints)
