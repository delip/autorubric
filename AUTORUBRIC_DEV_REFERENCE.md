...
| `MultiChoiceJudgeVote`     | Single judge's multi-choice vote (ensemble); optional `error` (category-prefixed message, parity with `JudgeVote`) + `is_error` property |
...
`compute_metrics` builds criteria from the author-space `dataset.rubric`, but a prediction may reference an **auto-injected** NA option at index `N = len(author.options)` (out of the author range). For each multi-choice criterion, metrics reconstructs the effective criterion via the same `Criterion.with_guaranteed_na_option()` helper **iff an out-of-range prediction is actually observed** — so injected-NA predictions are recognized (FP/FN, `na_kappa`, filtering) instead of crashing, while forced-choice runs (`auto_na_option=False`) are unaffected and never gain a spurious NA column. Consequence: once an ordinal criterion gains an NA option (authored *or* auto-injected and predicted), `na_mode="as_category"` is refused for it.
