...
- **Correlations.** `_compute_correlation` returns `CorrelationResult.coefficient = None` (and `p_value = None`) for `<3` samples (`interpretation="insufficient data"`) or a constant input array (NaN → `interpretation="undefined"`); `_interpret_correlation` is only ever called with a real float. The improvement loop's ground-truth Spearman (`validate_ground_truth`) likewise yields `None` on a constant rubric-score array.
