...
| `MultiChoiceJudgeVote` | Single judge's multi-choice vote (ensemble); optional `error` + `is_error` (parity with `JudgeVote`); optional `reasoning`. `selected_index`/`selected_label` are `int | None` / `str | None` — `None` for a genuine no-option abstain (see [Grading Flow](grading-flow.md)); same nullability on `MultiChoiceVerdict` / `AggregatedMultiChoiceVerdict`. Same additions as `JudgeVote`: `reason: str \| None`, `probabilities` (keys `str(original option index)`), `confidence`, `superseded: bool = False` — on the base vote type, so the `multi_choice_votes` list serializes them |
| `EnsembleCriterionReport` | Per-criterion aggregate of judge votes; optional `error` (set only when ALL votes errored) + `is_error`. `final_reason: str \| None` (vote reasons joined by `_join_vote_reasons`; `None` only when no vote carried a reason — the fixed reasons `"No votes"` and binary `"All judges could not assess"` are returned whatever the votes' reasons; see [Grading Flow](grading-flow.md)). `escalated: bool = False` — the first vote (a decision model's) was judged insufficient (errored, abstained, or low `confidence`), stays in the vote list with `superseded=True`, and the final verdict comes from the escalation judges' votes. The `agreement` validator (`_compute_agreement`) counts only non-`superseded` votes and keeps its "recompute only when `agreement == 0.0`" rule, so round-trips stay idempotent |
...
| Type | Purpose |
| --- | --- |
| `CriterionGrader` | Main grader - supports single judge (`judge_model_config=`, given `judge_id="default"`), ensemble (`judges=`), few-shot, and custom response formats. `llm_config=` is the deprecated alias of `judge_model_config` (last keyword-only parameter; see [Key Conventions](conventions.md)) |
| `JudgeSpec` | Ensemble judge config: llm_config, judge_id, weight. `llm_config` stays the **stored dataclass field** (never renamed), with `judge_model_config` as a read/write property over it. A hand-written `__init__` (typed by two `@overload`s; `dataclasses.MISSING` sentinels tell a missing argument from an explicit `None`) accepts the unchanged positional form `JudgeSpec(config, judge_id, weight=1.0)`, `llm_config=` (no warning) and `judge_model_config=` (preferred); both → `ValueError`, a missing config/`judge_id` → `TypeError` worded like Python's own. `fields()`/`asdict()`/`repr`/`__eq__`/`__match_args__`/`replace()` (which takes `llm_config=`)/pickles are unchanged — pinned by fixtures captured from the pre-alias library in `tests/golden/legacy/` |
...
| `JudgeMetrics` | Per-judge mirror of the aggregate (precision/recall/f1 binary MET-vs-rest → `None` for multi-choice-only). Adds `phi: float | None` (per-judge MCC on {MET, UNMET}) and `confusion_matrix: ConfusionMatrix | None` (per-judge, pooled across criteria from raw pre-filter codes; 3×3 with `CANNOT_ASSESS` abstain class last; `None` when no data). Score-level fields `score_rmse`/`score_mae` (`float \| None`), `score_spearman`/`score_kendall`/`score_pearson` (`CorrelationResult \| None`) and `bias` (`BiasResult \| None`) are all `None` when the judge has no defined `judge_scores` entry on any item; otherwise they use only the items where the entry is defined. Judges with every score defined always get non-`None` values — see [Metrics](metrics.md) |
